...
- Test files live in `tests/` (currently under `tests/Security/`). Keep new tests there, or in a sibling directory, so `phpunit.xml`'s `<testsuite>` picks them up.
...
- **Security:** Sanitize inputs using Cacti's input validation functions (`form_input_validate`, `get_filter_request_var`, etc.) and escape output with `html_escape`/`__esc`.
